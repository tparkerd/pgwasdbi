...
from pgwasdbi.util import insert
...
def find_species(conn, args, speciesShortname):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT species_id FROM species WHERE shortname = %s;", [speciesShortname]
    )
    row = cur.fetchone()
    if row is not None:
        speciesID = row[0]
...
def find_population(conn, args, populationName):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT population_id FROM population WHERE population_name = %s;",
        [populationName],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_line(conn, args, line_name, line_population):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT line_id FROM line WHERE line_name = %s AND line_population = %s;",
        (line_name, line_population),
    )
    row = cur.fetchone()
    if row is not None:
        lineID = row[0]
...
def find_growout_type(conn, args, growout_type):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT growout_type_id FROM growout_type WHERE growout_type = %s;",
        [growout_type],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_kinship_algorithm(conn, args, algorithm):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT kinship_algorithm_id FROM kinship_algorithm WHERE kinship_algorithm = %s;",
        [algorithm],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_population_structure_algorithm(conn, args, algorithm):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT population_structure_algorithm_id FROM population_structure_algorithm WHERE population_structure_algorithm = %s;",
        [algorithm],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_gwas_algorithm(conn, args, gwas_algorithm):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT gwas_algorithm_id FROM gwas_algorithm WHERE gwas_algorithm = %s;",
        [gwas_algorithm],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_genotype_version(conn, args, genotype_version_name):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT genotype_version_id FROM genotype_version WHERE genotype_version_name = %s;",
        [genotype_version_name],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_imputation_method(conn, args, imputation_method):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT imputation_method_id FROM imputation_method WHERE imputation_method = %s;",
        [imputation_method],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_kinship(conn, args, kinship_file_path):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT kinship_id FROM kinship WHERE kinship_file_path = %s;",
        [kinship_file_path],
    )
    row = cur.fetchone()
    if row is not None:
...
def find_population_structure(conn, args, population_structure_file_path):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT population_structure_id FROM population_structure WHERE population_structure_file_path = %s;",
        [population_structure_file_path],
    )
    row = cur.fetchone()
    if row is not None:

...
def validate_phenotype(args, filepath):
    ...
    if re.match(field_pattern, df.columns[0], re.IGNORECASE) is None:
        raise Exception(
            f"The name of the first column of a phenotype file should be one of the following values: {acceptable_fields}. Violating file: '{filepath}'"
        )
...
def validate_line(args, filepath):
    ...
    Args:
        ...
        filepath (str): location of input file
    """
    schema = Schema([Column("line_name", [IsDistinctValidation()])])

    df = pd.read_csv(filepath, header=None)

    if len(df.columns) != 1:
        raise Exception(
            f"Invalid file format. Excepted 1 column found {len(df.columns)} columns. This file should be a single column of each line. Each entry should be distinct."
        )
...
def validate_genotype(args, filepath):
    ...
    schema_columns = [
        Column("row_number", [CanConvertValidation(int) & IsDistinctValidation()])
    ]
    ...
    if not os.path.exists(pos_filepath):
        raise FileNotFoundError(
            f"Count not locate the position counterpart file for {filepath}"
        )
        ...
        schema_columns.append(
            Column(
                ...
                [
                    ...
                    & CustomSeriesValidation(
                        lambda x: x.int in [-1, 0, 1, 2], "Incorrectly coded value."
                    )
                ],
            )
        )
...
def validate_variant(args, filepath):
    ...
    schema = Schema(
        [
            Column("chr", [CanConvertValidation(int)]),
            Column("pos", [CanConvertValidation(int), IsDistinctValidation()]),
        ]
    )
    ...
    if len(df.columns) != 2:
        raise Exception(
            f"Invalid file format. Excepted 2 columns, found {len(df.columns)} columns. Columns should consist of chromsome number and SNP position. Filepath: {filepath}"
        )
...
def validate_kinship(args, filepath):
    ...
    schema_columns = [Column("line_name", [IsDistinctValidation()])]

    for n in range(1, ncols):
        schema_columns.append(Column(df.columns[n], [CanConvertValidation(float)]))
...
def validate_population_structure(args, filepath):
    ...
    schema_columns = [Column("Pedigree", [IsDistinctValidation()])]
...
async def validate_variant_sync(args, data):
    ...
    stmt = await conn.prepare(
        """SELECT variant_id FROM variant WHERE variant_species = $1 AND variant_chromosome = $2 AND variant_pos = $3"""
    )
    ...
    try:
        ...
        for c in range(1, conf["number_of_chromosomes"] + 1):
            ...
            genotype_filepath = genotype_filename.substitute(
                dict(
                    cwd=args.cwd, shortname=conf["species_shortname"], chr=chr_shortname
                )
            )
            ...
            if "genotype" in args.validate:
                locations.append(
                    dict(cwd=args.cwd, filetype="genotype", filename=genotype_filepath)
                )
            if "variant" in args.validate:
                locations.append(
                    dict(cwd=args.cwd, filetype="variant", filename=variants_filepath)
                )
        ...
        if "lines_filename" not in conf:
            ...
            lines_filepath = lines_filename.substitute(
                dict(cwd=args.cwd, shortname=conf["species_shortname"], chr="chr1")
            )
        else:
            lines_filepath = filepath_template.substitute(
                dict(cwd=args.cwd, filename=conf["lines_filename"])
            )
        conf["lines_filename"] = lines_filepath

        if "line" in args.validate:
            locations.append(
                dict(cwd=args.cwd, filetype="line", filename=lines_filepath)
            )
        ...
        if "phenotype_filename" in conf:
            ...
            conf["phenotype_filename"] = [
                filepath_template.substitute(
                    dict(cwd=args.cwd, filetype="phenotype", filename=pfp)
                )
                for pfp in conf["phenotype_filename"]
            ]
        if "population_structure_filename" in conf:
            conf["population_structure_filename"] = filepath_template.substitute(
                dict(
                    cwd=args.cwd,
                    filetype="population_structure",
                    filename=conf["population_structure_filename"],
                )
            )
        if "gwas_results_filename" in conf:
            ...
            conf["gwas_results_filename"] = [
                filepath_template.substitute(
                    dict(cwd=args.cwd, filetype="gwas_results_filename", filename=grfp)
                )
                for grfp in conf["gwas_results_filename"]
            ]
        if "gwas_run_filename" in conf:
            if not isinstance(conf["gwas_run_filename"], list):
                conf["gwas_run_filename"] = [conf["gwas_run_filename"]]
            conf["gwas_run_filename"] = [
                filepath_template.substitute(
                    dict(cwd=args.cwd, filetype="gwas_run_filename", filename=grfp)
                )
                for grfp in conf["gwas_run_filename"]
            ]
        if "kinship" in args.validate:
            locations.append(
                dict(
                    cwd=args.cwd, filetype="kinship", filename=conf["kinship_filename"]
                )
            )
                ...
                if configuration_entry in [
                    ...
                    locations.append(
                        dict(
                            cwd=args.cwd,
                            filetype=configuration_entry,
                            filename=conf[configuration_entry],
                        )
                    )
        ...
        for location in locations:
            if not os.path.isfile(location["filename"]):
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), location["filename"]
                )

        logging.info(
            f"Found all files found for validation steps: {args.validate}. Validating file contents."
        )
        configuration = {
            "chromosome": {"count": conf["number_of_chromosomes"]},
            "lines": {"file": conf["lines_filename"]},
            "kinship": {
            ...
            "variant": {"file": conf["variant_filename"]},
            "species": {
        ...
        args.chromosome = {"count": conf["number_of_chromosomes"]}
        args.lines = {"file": conf["lines_filename"]}
        args.kinship = {
            "file": conf["kinship_filename"],
            "algorithm": conf["kinship_algortihm_name"],
        }
        ...
        args.variant = {"file": conf["variant_filename"]}
        args.species = {
            "shortname": conf["species_shortname"],
            "binomial": conf["species_binomial_name"],
            "subspecies": conf["species_subspecies"],

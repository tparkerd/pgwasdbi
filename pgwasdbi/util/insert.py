...
import pgwasdbi.util.parsinghelpers as ph
from pgwasdbi.util.database import config
...
from pgwasdbi.util.validate import validate_variant_sync
...
def insert_variants_from_file(conn, args, variantPosFile, speciesID, chromosomeID):
    ...
    insertedVariantIDs = []
    for variantpos in tqdm(
        variantlist, desc="Variants from %s" % variantPosFile, total=cVariants
    ):
...
def insert_variants_from_file_async(
    conn, args, variantPosFile, speciesID, chromosomeID
):
    ...
    df = list(
        zip(([speciesID] * num_datapoints), ([chromosomeID] * num_datapoints), df)
    )
...
def insert_genotypes_from_file(
    conn, args, genotypeFile, lineFile, chromosomeID, populationID, genotype_versionID
):
    ...
    for zippedpair in tqdm(ziplist, desc="Genotypes from %s" % genotypeFile):
        genotypeObj = genotype(
            zippedpair[0], chromosomeID, zippedpair[1], genotype_versionID
        )
...
def insert_phenotypes_from_file(
    conn, args, phenotype_filename, population_id, filename
):
...
def insert_genotype_version(conn, args, genotype_version):
    ...
    SQL2 = """INSERT INTO genotype_version(genotype_version_assembly_name,
                                        genotype_version_annotation_name,
                                        reference_genome,
                                        genotype_version_population)
        VALUES ({0},{1},{2},{3})
        ON CONFLICT DO NOTHING
        RETURNING genotype_version_id;"""
    logging.debug(SQL2.format(*params))

    logging.debug(
        f"Inserting genotype version\n==========================\nSQL:\n{pformat(SQL)}\nParameters:\t{pformat(params)}"
    )
    cur.execute(SQL, params)
    row = cur.fetchone()
    if row is not None:
...
def insert_population_structure_algorithm(conn, args, population_structure_algorithm):
    ...
    if known_id is not None:
        logging.debug(
            f"[Population structure algorithm found] {population_structure_algorithm}"
        )
        conn.commit()
        cur.close()
        return known_id
...
def insert_gwas_runs_from_gwas_results_file(
    conn,
    args,
    gwas_results_file,
    gwasRunAlgorithmID,
...
def insert_gwas_results_from_file(
    conn,
    args,
    speciesID,
    gwas_results_file,
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="GWAS Results"):
        ...
        gwas_run_ID = find.find_gwas_run(
            conn=conn,
            args=args,
        ...
        if "bp" in df.columns:
            ...
            raise Exception(
                f"Cannot identify basepair column in GWAS result file: {gwas_results_file}"
            )
        ...
        new_gwas_result = gwas_result(
            gwas_result_chromosome=chromosomeID,
            basepair=basepair,
...
async def variant_sync_run(args, data):
    ...
    result = await conn.copy_records_to_table(
        "variant",
        columns=["variant_species", "variant_chromosome", "variant_pos"],
        records=data,
    )
